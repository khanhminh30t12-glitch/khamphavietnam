...
export function Map3DViewComponent({
  ...
  const renderLandmarkMarkers = (mapInstance: mapboxgl.Map) => {
    ...
    allLandmarks.forEach(landmark => {
      const isIsland = landmark.isIsland || landmark.id === 'lm_hoang_sa' || landmark.id === 'lm_truong_sa';
      const el = document.createElement('div');
      el.className = 'custom-mapbox-marker';
      el.style.width = isIsland ? '44px' : '38px';
      el.style.height = isIsland ? '44px' : '38px';
      el.style.borderRadius = '50%';
      el.style.backgroundColor = isIsland ? '#dc2626' : (regionColors[landmark.region] || '#0066CC');
      el.style.border = isIsland ? '3px solid #f59e0b' : '3px solid white';
      el.style.boxShadow = isIsland
        ? '0 0 22px rgba(239, 68, 68, 0.95), 0 0 12px rgba(245, 158, 11, 0.95)'
        : `0 0 15px ${regionColors[landmark.region]}80, 0 4px 14px rgba(0,0,0,0.5)`;
      el.style.cursor = 'pointer';
      el.style.display = 'flex';
      el.style.alignItems = 'center';
      el.style.justifyContent = 'center';
      el.style.fontSize = isIsland ? '22px' : '18px';
      el.style.transition = 'transform 0.2s cubic-bezier(0.16, 1, 0.3, 1)';
      el.innerHTML = isIsland ? '🇻🇳' : '📍';
